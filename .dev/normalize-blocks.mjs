...
// MONTANA_PATTERNS says where the patterns actually are in that case.
const patterns = process.env.MONTANA_PATTERNS
	? resolve( process.env.MONTANA_PATTERNS )
...
// `<?php ... ?>` -> `MTPHP0`, and back again after the round trip.
...
function restore( content, snippets ) {
	return content.replace( /MTPHP(\d+)X/g, ( _, i ) => snippets[ Number( i ) ] );
}
