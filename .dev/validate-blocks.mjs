...
const problems = await page.evaluate( async () => {
	...
	for ( const pattern of patterns ) {
		if ( ! pattern.name || 0 !== pattern.name.indexOf( 'montana/' ) ) {
			continue;
		}
	...
	// importer actually wrote, including the demo posts .dev/demo/import.php adds.
	for ( const type of [ 'pages', 'posts', 'navigation' ] ) {
...
console.log( 'Every block in every template, part, pattern, page, post and menu is valid.' );
