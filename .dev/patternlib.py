#!/usr/bin/env python3
"""Block markup helpers for Montana's pattern generators.
...
def separator(style="montana-bar", align=None):
    data = {"className": "is-style-" + style}
...
def cover(inner, slug, overlay="dark", dim=60, min_height=None, align="full",
    ...
    if gradient:
        # Verified against the serialiser: a custom-gradient cover still writes
        # the dim classes, then the two gradient classes.
        dim_cls = "" if dim == 0 else (" has-background-dim" if dim == 50 else
                                       " has-background-dim-%d has-background-dim" % (10 * round(dim / 10)))
        span = ('<span aria-hidden="true" class="wp-block-cover__background%s '
                'wp-block-cover__gradient-background has-background-gradient" style="background:%s"></span>'
                % (dim_cls, gradient))
    else:
    ...
    url = theme_image(slug)
    attr_json = attrs(data).replace('"MONTANA_IMAGE"', '"%s"' % url)
